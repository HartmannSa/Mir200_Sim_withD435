...
public:
    ...
    void createLearnImages(bool gray, bool depth, bool keypoints, std::string filename, std::string feature = "ORB", bool replaceLast=false){
        ...
        for (int i = 0; i < maxTrainImageNumber; i++) 
        {
            ...
            if (i < 2) {
                vpDisplay::displayText(I_color, 30, 10, "Click to learn next pose of the object...", vpColor::red);
            } else {
                 vpDisplay::displayText(I_color, 30, 10, "Click to finish learning.", vpColor::red);
            } 
            vpDisplay::flush(I_color);
            vpDisplay::getClick(I_color, true);        
        }
